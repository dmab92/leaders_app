...
class leaders_presence_personnel(models.Model):
    ...
    center_id = fields.Many2one("leaders.center", string="Centre de Prepas", required=True)
    date = fields.Date("Date du jour", default=datetime.today().date(), required=True)
    ...
    ligne_presence_ids = fields.One2many('leaders.ligne.presence', 'presence_id',
                                         string="  liste des presences")

    @api.depends("center_id", "date")
    def _generate_name(self):
        for l in self:
            l.name = 'Liste de Presence   De ' + str(l.center_id.name) + ' le  ' + fields.Date.from_string(l.date).strftime('%d/%m/%Y')
...
class leaders_ligne_presence(models.Model):
    ...
    employee_id = fields.Many2one('leaders.employee', "Nom du Personnel ",require=True, domain=[('type','in',('perma','temp'))])
    ...
    presence_id = fields.Many2one('leaders.presence.personnel')
